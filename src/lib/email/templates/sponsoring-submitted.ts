export function sponsoringSubmittedEmailTemplate(params: {
  companyName: string;
  profileKind: string;
  adminUrl: string;
  ...
  return {
    subject: `Nouvelle demande sponsoring ${kindLabel} — ${params.companyName} — vivasky.media`,
    html: `<!DOCTYPE html>
<html lang="fr">
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:480px;background:#FFFFFF;border-radius:8px;border:1px solid #E0E0E0;padding:40px 32px;">
        <tr><td>
          ...
          <h1 style="margin:0 0 8px;font-size:18px;font-weight:600;color:#242424;">Demande de sponsoring</h1>
          <p style="margin:0 0 20px;font-size:14px;color:#616161;line-height:1.5;">
            <strong>${params.companyName}</strong> a soumis une demande de campagne sponsorisée sur le moteur <strong>${kindLabel}</strong>.<br><br>
            La bannière et le lien doivent être vérifiés avant validation.
          </p>
