...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    const description = profile.pitch ? profile.pitch.substring(0, 160) : `${profile.company.displayName} sur vivasky.media`;
    return {
...
export default async function BrandUpProfilePage({ params }: PageProps): Promise<JSX.Element> {
  ...
  return (
    <div className="min-h-screen bg-[#fbf9f8]">
      <PublicProfileHeader product="brandup" />
      <TrackView profileId={(data! as PublicBrandUpProfile).profileId} />
