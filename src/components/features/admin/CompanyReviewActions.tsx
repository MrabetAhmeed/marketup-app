...
interface CompanyReviewActionsProps {
  companyId: string;
}

export function CompanyReviewActions({ companyId }: CompanyReviewActionsProps): JSX.Element {
  ...
  async function handleValidate(): Promise<void> {
    setValidating(true);
    try {
      const res = await fetch(`/api/v1/admin/companies/${companyId}/validate`, { method: "POST" });
      const json = await res.json();
      if (!res.ok) { showToast(json.error?.message || "Erreur"); return; }
      showToast("Compte validé et activé");
      router.push("/admin/validation/comptes");
      router.refresh();
  ...
  async function handleReject(reason: string): Promise<void> {
    setRejecting(true);
    try {
      const res = await fetch(`/api/v1/admin/companies/${companyId}/reject`, {
        method: "POST",
      ...
      const json = await res.json();
      if (!res.ok) { showToast(json.error?.message || "Erreur"); return; }
      showToast("Compte refusé — email envoyé");
      setRejectOpen(false);
      router.push("/admin/validation/comptes");
      router.refresh();
    ...
    <>
      <div className="flex items-center gap-3">
        ...
        <button type="button" disabled={busy} onClick={handleValidate}
          ...
          {validating ? "Validation…" : "Activer le compte"}
        </button>
      </div>
