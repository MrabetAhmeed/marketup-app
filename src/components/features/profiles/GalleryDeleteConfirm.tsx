...
export function GalleryDeleteConfirm({ open, onClose, caption, onConfirm }: GalleryDeleteConfirmProps): JSX.Element {
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-[420px] p-0 gap-0 overflow-hidden" showCloseButton={false}>
        <div className="flex items-start gap-3 px-5 py-4 border-b border-surface-border">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[12px] text-[#991B1B] mt-0.5">
              N&apos;oubliez pas d&apos;enregistrer pour confirmer la suppression
            </p>
          </div>
        </div>
        <div className="p-5">
          <p className="text-[13px] text-ink-secondary leading-relaxed">
            L&apos;image «&nbsp;{caption || "Sans titre"}&nbsp;» sera retirée de votre galerie lorsque vous cliquerez Enregistrer.
          </p>
        </div>
        <div className="px-5 py-4 bg-surface-subtle border-t border-surface-border flex items-center justify-end gap-2 rounded-b-xl">
          ...
          <button type="button" onClick={onConfirm} className="inline-flex items-center gap-1.5 px-4 py-[9px] text-[13px] font-semibold text-white bg-[#B91C1C] hover:bg-[#991B1B] rounded transition-colors">
