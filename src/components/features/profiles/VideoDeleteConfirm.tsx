...
interface VideoDeleteConfirmProps {
  open: boolean;
  onClose: () => void;
  profileId: string;
  videoId: string;
...
export function VideoDeleteConfirm({ open, onClose, profileId, videoId, title }: VideoDeleteConfirmProps): JSX.Element {
  ...
  async function handleDelete(): Promise<void> {
    setDeleting(true);
    try {
      const res = await fetch(`/api/v1/profiles/${profileId}/videos/${videoId}`, { method: "DELETE" });
      if (!res.ok) {
        const json = await res.json();
        showToast(json.error?.message || "Erreur lors de la suppression");
        return;
      }
      ...
      <DialogContent className="sm:max-w-[420px] p-0 gap-0 overflow-hidden" showCloseButton={false}>
        ...
        <div className="p-5">
          <p className="text-[13px] text-ink-secondary leading-relaxed">
            La vidéo «&nbsp;{title || "Sans titre"}&nbsp;» sera retirée de votre chaîne TraceUP.
          </p>
        </div>
        <div className="px-5 py-4 bg-surface-subtle border-t border-surface-border flex items-center justify-end gap-2 rounded-b-xl">
          ...
          <button type="button" disabled={deleting} onClick={handleDelete} className="inline-flex items-center gap-1.5 px-4 py-[9px] text-[13px] font-semibold text-white bg-[#B91C1C] hover:bg-[#991B1B] rounded transition-colors disabled:opacity-60">
            {deleting ? (
